...
`Host/App.config` selects `Card=alpha` and `FC=gamma` for S1. The host maps AppSettings into `IConfiguration`, then uses `DesignPatterns.Extensions.Configuration.RegistryConfiguration` to resolve strategy registries (`CardMissing=beta` drives S2).
